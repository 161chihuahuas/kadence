...
module.exports.getContactURL = function(contact) {
  ...
  return [
    'dusk',
    id.toString('hex'), 
    Buffer.from(info.hostname, 'utf8').toString('hex'),
    info.pubkey,
    Buffer.from([info.nonce]).toString('hex'),
    info.proof
  ].join('::');
};
...
module.exports.parseContactURL = function(addr) {
  const [ , fp, hostname, pubkey, nonce, proof] = addr.split('::');
  const contact = [fp, {
    protocol:'http:',
    hostname : Buffer.from(hostname, 'hex').toString('utf8'),
    port: 80,
...
module.exports.isValidContact = function(contact, loopback) {
  const [, info] = contact;
  const isValidAddr = onionRegex.v3({ exact: true }).test(info.hostname);
