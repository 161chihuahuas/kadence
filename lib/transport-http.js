...
class HTTPTransport extends DuplexStream {
  ...
  _timeoutPending() {
    ...
    this._pending.forEach(({ timestamp, response }, id) => {
      let timeout = timestamp + 
        constants.T_RESPONSETIMEOUT + HTTPTransport.T_TIMEOUTBUFFER;
  ...
  _handle(req, res) {
    ...
    if (!req.headers['x-dusk-message-id']) {
      res.statusCode = 400;
      return res.end();
    }

    res.setHeader('X-Dusk-Message-ID', req.headers['x-dusk-message-id']);
    ...
    req.pipe(concat((buffer) => {
      this._pending.set(req.headers['x-dusk-message-id'], {
        timestamp: Date.now(),
        response: res
      });
      this.push(buffer);
