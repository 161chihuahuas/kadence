...
class OnionPlugin {
  ...
  static get DEFAULTS() {
    return {
      dataDirectory: path.join(os.tmpdir(), 'dusk.dat'),
      virtualPort: 80,
      localMapping: '127.0.0.1:5274',
  ...
  /**
   * Returns a socket agent instance to use for the provided target
   * @returns {Agent}
   */
  createSocksAgent() {
  ...
  _wrapTransportRequest(transport) {
    ...
    transport._createRequest = (options) => {
      options.agent = this.createSocksAgent();
