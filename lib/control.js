...
class Control {
  ...
  getinfo(callback) {
    ...
    callback(null, {
      ...
      bundle: utils.getContactURL([this.node.identity, this.node.contact]),
      peers
    });
  }
  /**
  ...
  /**
   * {@link module:dusk/quasar~QuasarPlugin#quasarSubscribe}
   */
  /* istanbul ignore next */
  ...
  /**
   * {@link module:dusk/quasar~QuasarPlugin#quasarPublish}
   */
  /* istanbul ignore next */
  publish(hexKey, hexValue, callback) {
    if (arguments.length !== 3) {
      return arguments[arguments.length - 1](new Error('Invalid params'));
    }
    this.node.quasarPublish(hexKey, hexValue, callback);
  }
  ...
  bootstrap(seedString, callback) {
    if (arguments.length !== 2) {
      return arguments[arguments.length - 1](new Error('Invalid params'));
    }

    const [fingerprint, hostname] = seedString.split(':');
