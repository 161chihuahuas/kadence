...
const { keyStringIsValid, hash160 } = require('./utils');
...
class Protocol {
   ...
   */
  STORE(key, { meta, blob }, respond) {
    const kbuf = Buffer.from(key, 'hex');
    const hash = hash160(blob);
    ...
    this.events.emit('storage:put', key, { meta, blob }, respond);
  }
   ...
   */
  FIND_NODE(key, respond) {
    if (!keyStringIsValid(key)) {
      return respond(new Error('Invalid lookup key supplied'));
    }

    respond(null, [...this.router.getClosestContactsToKey(key).entries()]);
  }
   ...
   */
  FIND_VALUE(key, respond) {
    if (!keyStringIsValid(key)) {
      return respond(new Error('Invalid lookup key supplied'));
    }

    this.events.emit('storage:get', key, (err, item) => {
      if (err) {
        return this.FIND_NODE(key, respond);
      }
