...
class SpartacusPlugin {
  ...
  constructor(node, priv, opts) {
    ...
    node.contact.pubkey = Buffer.from(this.publicKey).toString('hex');
    ...
    node.rpc.deserializer.prepend(() => new Transform({
      transform: (buf, enc, callback) => {
        this.deserialize(buf, enc, (err, data) => {
          if (err) {
  ...
  serialize(data, encoding, callback) {
    ...
    let { signature, recovery } = secp256k1.ecdsaSign(
  ...
  deserialize(buffer, encoding, callback) {
    ...
    try {
      assert.ok(secp256k1.ecdsaVerify(
...
/**
 * Registers a {@link module:dusk/spartacus~SpartacusPlugin} with a
 * {@link KademliaNode}
